...
# ─── Mobile Client Models (ClawGuard) ────────────────────────────
...
class MobileConnectRequest(BaseModel):
    ...
    sandbox_name: Optional[str] = Field(None, alias="sandboxName")

    model_config = {"populate_by_name": True}

class MobileChatRequest(BaseModel):
    """Send a message from the mobile client with sentinel enforcement."""
...
class MobileChatResponse(BaseModel):
    ...
    blocked: bool = False
    audit_sequence: int = Field(0, alias="auditSequence")
...
class MobileSessionResponse(BaseModel):
    ...
    phi_detection: bool = Field(True, alias="phiDetection")
    block_on_suspicious: bool = Field(True, alias="blockOnSuspicious")
    audit_chain_valid: bool = Field(True, alias="auditChainValid")
    audit_chain_length: int = Field(0, alias="auditChainLength")
    version: str = "0.4.1"
