...
export class SqliteStore implements LcmStore {
  ...
  async backup(destinationPath: string): Promise<void> {
    if (!/^[a-zA-Z0-9._\-\/]+$/.test(destinationPath)) {
      throw new Error('Invalid backup path: only alphanumeric, dots, hyphens, underscores, slashes allowed');
    }
  ...
  async enforceRetention(cid: ConversationId): Promise<DeletionReport> {
    ...
    if (policy.maxMessages !== null) {
      ids.push(...this.q<{ id: string }>(
        'SELECT id FROM messages WHERE conversation_id = ? ORDER BY created_at ASC LIMIT max(0,(SELECT COUNT(*) FROM messages WHERE conversation_id = ?)-?)',
        cid, cid, policy.maxMessages).map(r => r.id));
    }
