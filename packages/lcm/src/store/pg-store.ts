...
export class PgStore implements LcmStore {
  ...
  async enforceRetention(cid: ConversationId): Promise<DeletionReport> {
    ...
    if (policy.maxAgeDays !== null) {
      const { rows } = await this.pool.query(
        "SELECT id FROM messages WHERE conversation_id = $1 AND created_at < NOW() - ($2 || ' days')::interval", [cid, policy.maxAgeDays]);
    ...
    if (policy.piiMaxAgeDays !== null) {
      const { rows } = await this.pool.query(
        "SELECT id FROM messages WHERE conversation_id = $1 AND pii_detected = TRUE AND created_at < NOW() - ($2 || ' days')::interval",
  ...
  async similaritySearch(vector: Float64Array, k: number, filter?: { entityType?: string }): Promise<SimilarityResult[]> {
    const vecStr = `[${Array.from(vector).join(',')}]`;
    let sql = `SELECT entity_type, entity_id, 1 - (vector <=> $1::vector) as score FROM embeddings WHERE TRUE`;
